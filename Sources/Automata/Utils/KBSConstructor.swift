...
public class KBSConstructor {
    ...
    public init(input_automata: Automata) {
        ...
        let new_initial_state = KBSConstructor.mergeStates(states: input_automata.initial_states)
    ...
    public func run() -> Automata {
        ...
        while (state_analyze_queue.count > 0) {
            ...
            repeat {
                ...
                let obs_eq_state_classes = KBSConstructor._divideStatesByAPs(input_states: old_states_successors)
                
                for obs_eq_state_set in obs_eq_state_classes {
                    // TODO: minor optimization: check before creating new state if it already exists
                    var obs_successor_state = KBSConstructor.mergeStates(states: obs_eq_state_set)
                    ...
                    let transition_formula = KBSConstructor._naiveBitsetToFormula(bs: current_bitset_condition, apList: self.obs_automata.apList)
