...
class FormulaTest: XCTestCase {
    ...
    func testBracketCorrectnessCheck() {
        let str1 = "(a) ∧ (b)"
        XCTAssertTrue(FormulaParser.checkBracketCorrectness(inputString: str1))

        let str2 = "((a)∨((bcd)) ∨ ((d) ∧ (a)))"
        XCTAssertTrue(FormulaParser.checkBracketCorrectness(inputString: str2))

        let str3 = "a∨((bcd)) ∨ (d ∧ a))"
        XCTAssertFalse(FormulaParser.checkBracketCorrectness(inputString: str3))

        let str4 = "((a∨((bcd)) ∨ (d ∧ a))"
        XCTAssertFalse(FormulaParser.checkBracketCorrectness(inputString: str4))

        let str5 = "(a∨((bcd) ∨ d) ∧ a)"
        XCTAssertFalse(FormulaParser.checkBracketCorrectness(inputString: str5))

        let str6 = "a∨(bcd ∨ (d ∧ a))"
        XCTAssertFalse(FormulaParser.checkBracketCorrectness(inputString: str6))

        let str7 = "(a∨bcd ∨ (d ∧ a))"
        XCTAssertTrue(FormulaParser.checkBracketCorrectness(inputString: str7))
    }
    ...
    func testdescription() {
        ...
        // Create Sample APs
        _ = AP(name: "a", observable: true, list: globalAPList)
        _ = AP(name: "b", observable: true, list: globalAPList)
        _ = AP(name: "c", observable: true, list: globalAPList)
        _ = AP(name: "d", observable: true, list: globalAPList)
        _ = AP(name: "test", observable: true, list: globalAPList)

        let str1 = "(a) ∧ (¬b)"
        let formula1 = FormulaParser.parseDNFFormula(inputString: str1, apList: globalAPList)
        ...
        let str2 = "((a)∨((¬test)) ∨ ((d) ∧ (a)))"
        XCTAssertTrue(FormulaParser.checkBracketCorrectness(inputString: str2))
        let formula2 = FormulaParser.parseDNFFormula(inputString: str2, apList: globalAPList)
        XCTAssertEqual(formula2!.description, "(a) ∨ (¬test) ∨ (d ∧ a)")

        let str3 = "(test ∨ (¬d ∧ ¬a))"
        XCTAssertTrue(FormulaParser.checkBracketCorrectness(inputString: str3))
        let formula3 = FormulaParser.parseDNFFormula(inputString: str3, apList: globalAPList)
    ...
    func testParseConjunctionLongerAps() {
        ...
        // Create Sample APs
        _ = AP(name: "a1", observable: true, list: globalAPList)
        _ = AP(name: "b2", observable: true, list: globalAPList, output: true)
        _ = AP(name: "c3", observable: true, list: globalAPList)
        _ = AP(name: "d4", observable: true, list: globalAPList, output: true)

        let conj1 = FormulaParser.parseConjunction(stringConjunction: "a1∧¬d4∧c3∧a1", apList: globalAPList)
        if conj1 == nil {
    ...
    func testParseConjunction() {
        ...
        let conj1 = FormulaParser.parseConjunction(stringConjunction: "a∧¬d∧c∧a", apList: globalAPList)
        if conj1 == nil {
            XCTAssert(false, "Unexpected Conjunction Parsing Error")
        } else {
        ...
        let conj2 = FormulaParser.parseConjunction(stringConjunction: "a∧¬false∧c∧a", apList: globalAPList)
        if conj2 == nil {
            XCTAssert(false, "Unexpected Conjunction Parsing Error")
        } else {
        ...
        let conj3 = FormulaParser.parseConjunction(stringConjunction: "a∧¬d1∧c∧a", apList: globalAPList)
        if conj3 != nil {
            XCTAssert(false, "Expected Conjunction Parsing Error")
        }

        let conj4 = FormulaParser.parseConjunction(stringConjunction: "a∧¬a∧c∧f", apList: globalAPList)
        if conj4 != nil {
            XCTAssert(false, "Expected Conjunction Parsing Error")
        }

        let conj5 = FormulaParser.parseConjunction(stringConjunction: "a∧¬a∧c∧∧a", apList: globalAPList)
        if conj5 != nil {
            XCTAssert(false, "Expected Conjunction Parsing Error")
        }

        let conj6 = FormulaParser.parseConjunction(stringConjunction: "a∧¬a∧c∧a∧", apList: globalAPList)
        if conj6 != nil {
            XCTAssert(false, "Expected Conjunction Parsing Error")
        }
    ...
    func testParseDNFFormula() {
        ...
        let formula1 = FormulaParser.parseDNFFormula(inputString: "((a) ∨ (b) ∨ (¬d))", apList: globalAPList)
        if formula1 == nil {
        ...
        let formula2 = FormulaParser.parseDNFFormula(inputString: "((false) ∨ (b) ∨ (¬true))", apList: globalAPList)
        if formula2 == nil {
            XCTAssert(false, "Unexpected Formula Parsing Error")
        } else {
        ...
        let formula3 = FormulaParser.parseDNFFormula(inputString: "((a ∧ ¬c ∧ ¬false) ∨ (b ∧ b) ∨ (¬d))", apList: globalAPList)
        if formula3 == nil {
            XCTAssert(false, "Unexpected Formula Parsing Error")
        } else {
        ...
        let formula4 = FormulaParser.parseDNFFormula(inputString: "((a ∧ ¬c ∧ ¬false) ∨ ∨  (b ∧ b) ∨ (¬d))", apList: globalAPList)
        if formula4 != nil {
            XCTAssert(false, "Expected Formula Parsing Error")
        }

        let formula5 = FormulaParser.parseDNFFormula(inputString: "", apList: globalAPList)
        if formula5 == nil {
            XCTAssert(false, "Unexpected Formula Parsing Error")
        } else {
        ...
        let formula6 = FormulaParser.parseDNFFormula(inputString: "((a ∧ ¬c ∧ ¬false) ∨ (b ∧ b) ∨ (¬d) ∨ )", apList: globalAPList)
        if formula6 != nil {
            XCTAssert(false, "Expected Formula Parsing Error")
        }
    }
    ...
    func testSimplifyFormula() {
        ...
        var formula1_0 = FormulaParser.parseDNFFormula(inputString: "((a) ∨ (o1) ∨ (¬d))", apList: globalAPList)
        var true_aps = [ap_a]
        formula1_0!.simplifyWithConstants(trueAPs: true_aps)
        XCTAssertEqual(formula1_0!.description, "(true) ∨ (o1) ∨ (¬false)")

        var formula1_1 = FormulaParser.parseDNFFormula(inputString: "((a) ∨ (o1) ∨ (¬d))", apList: globalAPList)
        true_aps = [ap_a, ap_d, ap_b]
        formula1_1!.simplifyWithConstants(trueAPs: true_aps)
        XCTAssertEqual(formula1_1!.description, "(true) ∨ (o1) ∨ (¬true)")

        var formula1_2 = FormulaParser.parseDNFFormula(inputString: "((a) ∨ (o1) ∨ (¬d))", apList: globalAPList)
        true_aps = [ap_b]
        formula1_2!.simplifyWithConstants(trueAPs: true_aps)
        XCTAssertEqual(formula1_2!.description, "(false) ∨ (o1) ∨ (¬false)")

        var formula2_0 = FormulaParser.parseDNFFormula(inputString: "((false) ∨ (b) ∨ (¬true))", apList: globalAPList)
        true_aps = [ap_a, ap_d, ap_b]
        formula2_0!.simplifyWithConstants(trueAPs: true_aps)
        XCTAssertEqual(formula2_0!.description, "(false) ∨ (true) ∨ (¬true)")

        var formula2_1 = FormulaParser.parseDNFFormula(inputString: "((o1) ∨ (b) ∨ (¬true))", apList: globalAPList)
        true_aps = [ap_a, ap_d]
        formula2_1!.simplifyWithConstants(trueAPs: true_aps)
